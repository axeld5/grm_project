...
def evaluate_models(list_of_reviews, num_node_features, num_classes, device, epochs=10, 
    ...
    history_dict = {"gcn":np.zeros((num_loops, epochs)), "gat":np.zeros((num_loops, epochs)), "graphsage":np.zeros((num_loops, epochs))}
    for i in range(num_loops):
        train_data, test_data = train_test_split(list_of_reviews, test_size=0.1)
        train_loader = DataLoader(train_data, batch_size=batch_size, shuffle=True)
        test_loader = DataLoader(test_data, batch_size=batch_size, shuffle=False)        
        final_gcn_accuracy, gcn_history, final_gat_accuracy, gat_history, final_gsage_accuracy, gsage_history = evaluate_loop(train_loader, test_loader,
...
def evaluate_loop(train_loader, test_loader, num_node_features, num_classes, device, epochs=10, gcn_hidden_size=256, gat_hidden_size=128, gsage_hidden_size=300):
    ...
    return final_gcn_accuracy, gcn_history, final_gat_accuracy, gat_history, final_gsage_accuracy, gsage_history

def evaluate_edge_model(list_of_reviews, num_node_features, num_classes, device, epochs=10, 
                    gcn_hidden_size=256, gat_hidden_size=128, gsage_hidden_size=300, num_loops=5, batch_size=1):
    perf_dict = {"model_name":[], "accuracy":[]}
    history_dict = {"gat":np.zeros((num_loops, epochs)), "directed_edgegat":np.zeros((num_loops, epochs))}
    for i in range(num_loops):
        train_data, test_data = train_test_split(list_of_reviews, test_size=0.1)
        train_loader = DataLoader(train_data, batch_size=batch_size, shuffle=True)
        test_loader = DataLoader(test_data, batch_size=batch_size, shuffle=False)  
...
def evaluate_all_edge_models(di_list_of_reviews, undi_list_of_reviews, wtd_list_of_reviews, num_node_features, num_classes, device, epochs=10, 
                    gcn_hidden_size=256, gat_hidden_size=128, gsage_hidden_size=300, num_loops=5, batch_size=1):
    perf_dict = {"model_name":[], "accuracy":[]}
    history_dict = {"gat":np.zeros((num_loops, epochs)), "directed_edgegat":np.zeros((num_loops, epochs)), "undirected_edgegat":np.zeros((num_loops, epochs)), 
                    "weighted_edgegat":np.zeros((num_loops, epochs))}
    seed_list = [13, 42, 200, 500, 2000]
    for i in range(num_loops):
        di_train_data, di_test_data = train_test_split(di_list_of_reviews, test_size=0.1, random_state=seed_list[i])
...
def evaluate_edge_loop(train_loader, test_loader, num_node_features, num_classes, edge_dim, device, 
                       epochs=10, gat_hidden_size=128, train_gat=True):
    gat_history = np.zeros(epochs)
    if train_gat:
        gat_model = GAT(num_node_features, gat_hidden_size, num_classes, dropout=0.2).to(device)
        for epoch in range(epochs):
