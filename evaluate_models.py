...
from models.gcn import GCN, MyGCN
...
def evaluate_loop(
    ...
    gcn_model = GCN(num_node_features, gcn_hidden_size, num_classes, dropout=0.2).to(
        device
    )
    ...
    gat_model = GAT(num_node_features, gat_hidden_size, num_classes, dropout=0.2).to(
        device
    )
    ...
    gsage_model = GraphSAGE(num_node_features, gsage_hidden_size, num_classes).to(
        device
    )
    ...
    my_gcn_model = MyGCN(
        num_node_features, gcn_hidden_size, num_classes, dropout=0.2
    ).to(device)
...
def evaluate_all_edge_models(
    di_list_of_reviews,
    undi_list_of_reviews,
    wtd_list_of_reviews,
    num_node_features,
    num_classes,
    device,
    ...
    for i in range(num_loops):
        ...
        undi_train_data, undi_test_data = train_test_split(
            undi_list_of_reviews, test_size=0.1, random_state=seed_list[i]
        )
        undi_train_loader = DataLoader(
            undi_train_data, batch_size=batch_size, shuffle=True
        )
        undi_test_loader = DataLoader(
            undi_test_data, batch_size=batch_size, shuffle=False
        )
        edge_dim = 19
        _, _, final_undi_edge_gat_accuracy, undi_edge_gat_history = evaluate_edge_loop(
            undi_train_loader,
            undi_test_loader,
            num_node_features,
            num_classes,
        ...
        wtd_train_data, wtd_test_data = train_test_split(
            wtd_list_of_reviews, test_size=0.1, random_state=seed_list[i]
        )
        wtd_train_loader = DataLoader(
            wtd_train_data, batch_size=batch_size, shuffle=True
        )
        wtd_test_loader = DataLoader(
            wtd_test_data, batch_size=batch_size, shuffle=False
        )
        edge_dim = 1
        _, _, final_wtd_edge_gat_accuracy, wtd_edge_gat_history = evaluate_edge_loop(
            wtd_train_loader,
            wtd_test_loader,
            num_node_features,
            num_classes,
...
def evaluate_edge_loop(
    ...
    if train_gat:
        gat_model = GAT(
            num_node_features, gat_hidden_size, num_classes, dropout=0.2
        ).to(device)
        for epoch in range(epochs):
            loss = train(gat_model, train_loader, device)
            train_acc = test(gat_model, train_loader, device)
    ...
    edge_gat_model = EdgeGAT(
        num_node_features, gat_hidden_size, num_classes, edge_dim, dropout=0.2
    ).to(device)
