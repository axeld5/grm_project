...
from models.gcn import GCN, EdgeGCN
...
def evaluate_models(list_of_reviews, num_node_features, num_classes, device, epochs=10, 
    ...
    for _ in range(num_loops):
        ...
        final_edge_gcn_accuracy, final_edge_gat_accuracy = evaluate_edge_loop(train_loader, test_loader,
                                            num_node_features, num_classes, device, epochs, gcn_hidden_size, gat_hidden_size, gsage_hidden_size)
        perf_dict["model_name"].append("gcn")
        perf_dict["accuracy"].append(final_gcn_accuracy)
        perf_dict["model_name"].append("edge_gcn")
        perf_dict["accuracy"].append(final_edge_gcn_accuracy)
...
def evaluate_edge_loop(train_loader, test_loader, num_node_features, num_classes, device, epochs=10, gcn_hidden_size=256, gat_hidden_size=128, gsage_hidden_size=300):
    ...
    edge_gcn_model = EdgeGCN(num_node_features, gcn_hidden_size, num_classes, dropout=0.2).to(device)
    for epoch in range(epochs):
        loss = edge_train(edge_gcn_model, train_loader, device)
        train_acc = edge_test(edge_gcn_model, train_loader, device)
        test_acc = edge_test(edge_gcn_model, test_loader, device)
        print(f"Epoch: {epoch:03d}, Loss: {loss:.4f}, Train Acc: {train_acc:.4f}", f"Test Acc: {test_acc:.4f}")
    final_edge_gcn_accuracy = edge_test(edge_gcn_model, test_loader, device)*100
    print(f'\nEdge GCN test accuracy: {final_edge_gcn_accuracy:.2f}%\n')
    ...
    return final_edge_gcn_accuracy, final_edge_gat_accuracy